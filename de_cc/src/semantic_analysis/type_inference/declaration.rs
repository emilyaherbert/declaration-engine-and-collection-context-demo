...
fn analyze_variable(
    ...
    resolve_custom_types(var_decl.type_ascription, cc, current_index).unwrap();
...
fn analyze_struct(cc: &CollectionContext, decl_id: &mut CCIdx<DeclarationId>) {
    let struct_decl = de_get_struct(*decl_id.inner_ref()).unwrap();
...
fn analyze_trait(cc: &CollectionContext, decl_id: &mut CCIdx<DeclarationId>) {
    ...
    trait_decl
        .interface_surface
        .iter_mut()
        .for_each(|trait_fn_id| analyze_trait_fn(cc, trait_fn_id));
}

fn analyze_trait_fn(cc: &CollectionContext, decl_id: &mut CCIdx<DeclarationId>) {
