...
#[proc_macro_derive(DebugWithCC, attributes(in_context))]
    ...
    let fmt_fields = {
        fields_named
            ...
            .map(|(field, name)| {
                ...
                let expr = pass_through_cc(name, &field.attrs);
                quote! {
...
fn pass_through_cc(field_name: &Ident, attrs: &[Attribute]) -> proc_macro2::TokenStream {
    ...
    match context_field_opt {
        None => {
            quote! {
                #field_name.with_cc(cc)
            }
        }
        Some(context_field) => {
            quote! {
                cc.#context_field[*#field_name].with_cc(cc)
            }
        }
    }
}
