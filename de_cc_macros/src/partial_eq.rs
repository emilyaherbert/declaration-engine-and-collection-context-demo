...
pub(super) fn derive_partial_eq_with_cc_inner(input: TokenStream) -> TokenStream {
    let DeriveInput {
        ident,
        generics,
        data,
    ...
    let body = match data {
        ...
        Data::Union(_) => {
            panic!("#[derive(PartialEqWithCC)] cannot be used on unions");
        }
    };
    let output = quote! {
        impl #impl_generics PartialEqWithCC for #ident #ty_generics
        #where_clause
        {
            fn fmt_with_cc<'a, 'c>(
                &'a self,
